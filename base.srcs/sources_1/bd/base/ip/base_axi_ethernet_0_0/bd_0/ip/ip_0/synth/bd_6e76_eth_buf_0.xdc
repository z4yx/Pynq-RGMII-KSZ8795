...
set_max_delay -from [get_cells {*RCV_INTFCE_I/RX_AXISTREAM_IF_I/rxs_mem_last_read_out_ptr_gray_d1_reg[*]}] -to [get_cells {*RCV_INTFCE_I/INCLUDE_RX_VLAN.RX_EMAC_IF_I/SYNC_RXS_LAST_READ_GRAY_PROCESS[*].SYNC_RXS_LAST_READ_GRAY/data_sync0_i}]  8 -datapath_only
set_max_delay -from [get_cells {*RCV_INTFCE_I/RX_AXISTREAM_IF_I/rxd_mem_last_read_out_ptr_gray_d1_reg[*]}] -to [get_cells {*RCV_INTFCE_I/INCLUDE_RX_VLAN.RX_EMAC_IF_I/SYNC_RXD_LAST_READ_GRAY_PROCESS[*].SYNC_RXD_LAST_READ_GRAY/data_sync0_i}]  8 -datapath_only

set_false_path -through [get_nets -hier -regexp {.*speed_is_10_100.*} ]


set_max_delay -from  [get_cells -hier -nocase -regexp {.*I_REGISTERS/RX_VLAN_BRAM.I_RX_VLAN_MEM/.*} -filter {IS_SEQUENTIAL=="1"}] -to [get_cells -hier -nocase -regexp {.*I_REGISTERS/IP2Bus_Data_reg.*}  -filter {IS_SEQUENTIAL=="1"}] 8 -datapath_only
set_max_delay -from  [get_cells -hier -nocase -regexp {.*I_REGISTERS/TX_VLAN_BRAM.I_TX_VLAN_MEM/.*} -filter {IS_SEQUENTIAL=="1"}] -to [get_cells -hier -nocase -regexp {.*I_REGISTERS/IP2Bus_Data_reg.*}  -filter {IS_SEQUENTIAL=="1"}] 8 -datapath_only
